...
const donorSchema = new mongoose.Schema({
  ...
  source: {
    type: String,
    default: 'website',
    enum: ['website', 'mobile_app', 'social_media', 'email_campaign', 'referral', 'other']
  },
  utmSource: String,
  utmMedium: String,
  utmCampaign: String
...
donorSchema.virtual('donationFrequency').get(function() {
  ...
  const daysSinceFirst = (new Date() - this.firstDonationDate) / (1000 * 60 * 60 * 24);
  const yearsSinceFirst = daysSinceFirst / 365.25;
  
  return yearsSinceFirst > 0 ? Math.round(this.donationCount / yearsSinceFirst) : this.donationCount;
});
...
donorSchema.pre('save', function(next) {
  ...
  if (this.isModified('donationCount') || this.isModified('totalDonated')) {
    ...
    } else if (this.activeSubscriptions && this.activeSubscriptions.length > 0) {
